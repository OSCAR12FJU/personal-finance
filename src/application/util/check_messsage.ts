export const checkStringText = (message:string): string[]=>{
...
export const checkNumberText = (arrayMessage: string[]): number =>{
...
export const validateInput = (text: string) =>{
  try{
    ...
    if(!arrayMessage.length) throw new Error('Formato incorrecto. Usa: "Descripción y monto"');
  
    const amount = checkNumberText(arrayMessage)
    if(isNaN(amount)) throw new Error('El monto debe ser número válido');
